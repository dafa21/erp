...
async function main() {
  try {
    const completion = await openai.chat.completions.create({
      ...
      top_p: 0.95,
      max_tokens: 100,
    } as any, { body: { chat_template_kwargs: { thinking: false } } });
    console.log("With body override:", completion.choices[0].message.content);
  } catch (error) {
    console.error("Error with body override:", error.message);
  }
